...
suspend inline fun <reified T> HttpClient.safePut(url: String, block: HttpRequestBuilder.() -> Unit = {} ): ApiResult<T> = try {
...
suspend inline fun <reified T> HttpClient.safeDelete(url: String, block: HttpRequestBuilder.() -> Unit = {}): ApiResult<T> = try {
    val response: HttpResponse = delete(url) {
        block()
    }
    handleResponse<T>(response)
} catch (e: NoTransformationFoundException) {
