...
object RegistryGenerator {
    ...
    private fun generateInsertOrReplaceFunction(): FunSpec {
        ...
        val codeBlock = CodeBlock.builder()
            ...
            .addStatement("val values = metadata.extractValues(entity)")
            .addStatement("val pk = metadata.primaryKey")
            .addStatement("val pkValue = values[pk.name]")
            .addStatement("")
            .addStatement("// Determine which columns to include based on PK value")
            .addStatement("val insertColumns = if (pkValue == null || pkValue == 0L || pkValue == 0) {")
            .indent()
            .addStatement("// New record: exclude auto-increment PK")
            .addStatement("metadata.columns.filterNot { it.isAutoIncrement }")
            .unindent()
            .addStatement("} else {")
            .indent()
            .addStatement("// Existing record: include PK for update")
            .addStatement("metadata.columns")
            .unindent()
            .addStatement("}")
            .addStatement("")
            .addStatement("val columnNames = insertColumns.joinToString(\", \") { it.name }")
