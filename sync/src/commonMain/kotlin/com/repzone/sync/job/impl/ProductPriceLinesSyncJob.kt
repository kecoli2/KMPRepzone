...
import com.repzone.core.constant.IPriceListApiControllerConstant
...
class ProductPriceLinesSyncJob(apiService: ISyncApiService<List<SyncProductPriceLinesDto>>,
                               bulkinsertService: IBulkInsertService<List<SyncProductPriceLinesDto>>,
                               syncModuleRepository: ISyncModuleRepository,
): BasePaginatedSyncJob<List<SyncProductPriceLinesDto>>(apiService, bulkinsertService, syncModuleRepository) {
    //region Field
    override val allowedRoles = setOf(UserRole.SALES_REP)
    override val jobType = SyncJobType.PRODUCT_PRICE_LINE
    override val defaultRequestEndPoint = IPriceListApiControllerConstant.PRICE_LINE_ENDPOINT
    ...
    override fun getFetchedMessage(count: Int): ResourceUI {
        return ResourceUI(
            ...
            args = listOf(count, Res.string.job_product_prices_lines)
        )
    }
    ...
    override fun extractLastIdAndLastDate(dtoData: List<SyncProductPriceLinesDto>, requestModel: FilterModelRequest?){
    ...
    override fun getDataSize(dtoData: List<SyncProductPriceLinesDto>): Int {
        return dtoData.size
    }
