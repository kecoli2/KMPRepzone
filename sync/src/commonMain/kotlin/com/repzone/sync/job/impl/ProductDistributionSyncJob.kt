...
class ProductDistributionSyncJob(apiService: ISyncApiService<List<SyncProductDistributionDto>>,
                                 bulkinsertService: IBulkInsertService<List<SyncProductDistributionDto>>,
                                 syncModuleRepository: ISyncModuleRepository,
): BasePaginatedSyncJob<List<SyncProductDistributionDto>>(apiService, bulkinsertService, syncModuleRepository) {
    //region Field
    override val allowedRoles = setOf(UserRole.SALES_REP)
    override val jobType = SyncJobType.DISTRIBUTIONS_PRODUCT
    override val defaultRequestEndPoint = IDistributionApiControllerConstant.PRODUCT_DISTRIBUTION_PRODUCT_ENDPOINT
    ...
    override fun getFetchedMessage(count: Int): ResourceUI {
        return ResourceUI(
            ...
            args = listOf(count, Res.string.job_customer_distribution_product)
        )
    }
    ...
    override fun extractLastIdAndLastDate(dtoData: List<SyncProductDistributionDto>, requestModel: FilterModelRequest?){
    ...
    override fun getDataSize(dtoData: List<SyncProductDistributionDto>): Int {
        return dtoData.size
    }
