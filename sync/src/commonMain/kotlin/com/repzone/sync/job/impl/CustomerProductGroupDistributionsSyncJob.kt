...
class CustomerProductGroupDistributionsSyncJob(apiService: ISyncApiService<List<SyncCustomerGroupProductDistributionDto>>,
                                               bulkinsertService: IBulkInsertService<List<SyncCustomerGroupProductDistributionDto>>,
                                               syncModuleRepository: ISyncModuleRepository,
): BasePaginatedSyncJob<List<SyncCustomerGroupProductDistributionDto>>(apiService, bulkinsertService, syncModuleRepository) {
    //region Field
    override val allowedRoles = setOf(UserRole.SALES_REP)
    override val jobType = SyncJobType.DISTRIBUTIONS_CUSTOMER_PRODUCT_GROUP
    override val defaultRequestEndPoint = IDistributionApiControllerConstant.CUSTOMER_PRODUCT_GROUP_DISTRIBUTION_ENDPOINT
    ...
    override fun getFetchedMessage(count: Int): ResourceUI {
        return ResourceUI(
            ...
            args = listOf(count, Res.string.job_customer_group_distribution)
        )
    }
    ...
    override fun extractLastIdAndLastDate(dtoData: List<SyncCustomerGroupProductDistributionDto>, requestModel: FilterModelRequest?){
    ...
    override fun getDataSize(dtoData: List<SyncCustomerGroupProductDistributionDto>): Int {
        return dtoData.size
    }
