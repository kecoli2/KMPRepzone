...
class PriceListSyncJob(apiService: ISyncApiService<List<SyncProductPricesDto>>,
                       bulkinsertService: IBulkInsertService<List<SyncProductPricesDto>>,
                       syncModuleRepository: ISyncModuleRepository,
): BasePaginatedSyncJob<List<SyncProductPricesDto>>(apiService, bulkinsertService, syncModuleRepository) {
    //region Field
    override val allowedRoles = setOf(UserRole.SALES_REP)
    override val jobType = SyncJobType.PRODUCT_PRICE_LIST
    override val defaultRequestEndPoint = IPriceListApiControllerConstant.PRICE_LIST_ENDPOINT
    ...
    override fun getFetchedMessage(count: Int): ResourceUI {
        return ResourceUI(
            ...
            args = listOf(count, Res.string.job_product_prices)
        )
    }
    ...
    override fun extractLastIdAndLastDate(dtoData: List<SyncProductPricesDto>, requestModel: FilterModelRequest?){
    ...
    override fun getDataSize(dtoData: List<SyncProductPricesDto>): Int {
        return dtoData.size
    }
