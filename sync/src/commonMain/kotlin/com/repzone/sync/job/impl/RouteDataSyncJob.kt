...
class RouteDataSyncJob(apiService: ISyncApiService<List<MobileRouteDto>>,
                           bulkInsertService: IBulkInsertService<List<MobileRouteDto>>,
                           syncModuleRepository: ISyncModuleRepository,
): BasePaginatedSyncJob<List<MobileRouteDto>>(apiService, bulkInsertService, syncModuleRepository) {
    //region Field
    ...
    override fun extractLastId(dtoData: List<MobileRouteDto>): Long {
        return dtoData.lastOrNull()?.id?.toLong() ?: 0L
    }

    override fun getDataSize(dtoData: List<MobileRouteDto>): Int {
        return dtoData.size
    }
